...
import { db } from "./db";
import { 
  participants, questions, submissions, admins, settings,
  type Participant, type InsertParticipant,
  type Question, type InsertQuestion,
  type Submission, type Admin,
  type LeaderboardEntry, type Setting
} from "@shared/schema";
...
export interface IStorage {
  ...
  getParticipantWrongAttemptCount(participantId: number, questionId: number): Promise<number>;
  ...
  getLeaderboard(type: 'daily' | 'monthly', date?: string): Promise<LeaderboardEntry[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getCurrentResetId(): Promise<number> {
    ...
    if (!s) {
      const [newS] = await db.insert(settings).values({ currentResetId: 0 }).returning();
  ...
  async performReset(): Promise<number> {
    ...
    const [s] = await db.select().from(settings).limit(1);
    if (s) {
      await db.update(settings).set({ currentResetId: newId, lastResetAt: new Date() }).where(eq(settings.id, s.id));
    } else {
      await db.insert(settings).values({ currentResetId: newId, lastResetAt: new Date() });
    }
    return newId;
  }
  ...
  async createQuestion(question: InsertQuestion): Promise<Question> {
    const resetId = await this.getCurrentResetId();
    const [q] = await db.insert(questions).values({ ...question, resetId }).returning();
    return q;
  }
  ...
  async createSubmission(submission: Partial<Submission>): Promise<Submission> {
    const resetId = await this.getCurrentResetId();
    // @ts-ignore
    const [s] = await db.insert(submissions).values({ ...submission, resetId }).returning();
    return s;
  }
  ...
  async getLeaderboard(type: 'daily' | 'monthly', date?: string): Promise<LeaderboardEntry[]> {
    const resetId = await this.getCurrentResetId();
    
    const conditions = [eq(submissions.resetId, resetId)];
